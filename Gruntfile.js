...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		rsync: {
			options: {
				...
				exclude: [".git*", "scss", "node_modules", "Gruntfile.js", "package.json", ".sass-cache*", "main.css.map"],
				recursive: true
			},
			//Pushes content from local to remote website
			push: {
				options: {
					src: "../../",
			...
			//Pulls content from remote site
			pull: {
				options: {
					ssh: true,
		...
		sshexec: {
			dump_remote_db: {
				...
				command: 'cd <%= mysql.remote.save_path %> && mysqldump <%= mysql.remote.dbname %> -u <%= mysql.remote.dbuser %> -p<%= mysql.remote.dbpass %> > remote.sql'
			},
			cleanup_remote: {
				options: {
					host: '<%= mysql.remote.host %>',
				...
				command: 'cd <%= mysql.remote.save_path %> && rm local_migrated.sql'
			},
			cleanup_remote_dump: {
				options: {
					host: '<%= mysql.remote.host %>',
				...
				command: 'cd <%= mysql.remote.save_path %> && rm remote.sql'
			},
			import_migrated_local_dump: {
				...
				command: 'cd <%= mysql.remote.save_path %> && mysql -u <%= mysql.remote.dbuser %> -p<%= mysql.remote.dbpass %> <%= mysql.remote.dbname %> < local_migrated.sql'
			},
			search_replace_remote: {
				...
				command: 'cd <%= mysql.remote.save_path %> && mysql -u <%= mysql.remote.dbuser %> -p<%= mysql.remote.dbpass %> <%= mysql.remote.dbname %> < local.sql'
			},
		},
		exec: {
			wget_remote_dump: {
				command: 'wget -nv <%= mysql.remote.save_url %>/remote.sql'
			},
			import_migrated_remote_dump: {
				command: 'vagrant ssh -c "cd <%= mysql.local.dump_dir %> && mysql -u <%= mysql.local.dbuser %> -p<%= mysql.local.dbpass %> <%= mysql.local.dbname %> < remote_migrated.sql"'
			},
			cleanup_local: {
				command: 'rm -rf <%= mysql.local.local_dump_dir %>/local.sql && rm -rf <%= mysql.local.local_dump_dir %>/local_migrated.sql'
			},
			cleanup_local_from_remote: {
				command: 'rm -rf remote.sql remote.sql && rm -rf remote_migrated.sql'
			},
			dump_local_db: {
				command: 'vagrant ssh -c "mysqldump -u <%= mysql.local.dbuser %> -p<%= mysql.local.dbpass %> <%= mysql.local.dbname %> > <%= mysql.local.dump_dir %>local.sql"'
			},
			scp_local_dump: {
				command: 'scp -P <%= mysql.remote.port %> <%= mysql.local.local_dump_dir %>local_migrated.sql <%= mysql.remote.username %>@<%= mysql.remote.host %>:<%= mysql.remote.save_path %>'
			},
	...
	grunt.registerTask('push', ['rsync:push']);
